...
SELECT person.first_name,
  ...
  FROM    (   (   birus.user user
           ...
           ON (site_to_site_req.hoc_approved_by = user.id))
       INNER JOIN
          birus.site site
       ON (site_to_site_req.site_id = site.id)
	   where site_to_site_req.site_to_id=".$_SESSION['site_id']."
	   AND
	   site_to_site_req.approved_issue_date !='00-00-00 00:00:00'
	   AND
	   site_to_site_req.hoc_approved_date != '0000-00-00 00:00:00'
	   AND
	   site_to_site_req.approved_date != '0000-00-00 00:00:00'
";
...
<?php
if(isset($_REQUEST['site_to_site_issue_approve']) && $_REQUEST['site_to_site_issue_approve']==true){
	$approve_update="UPDATE birus.site_to_site_req SET approved_issue_date = '".date("y-m-d")."', approved_issue_by = '".$loggedin_id."', approved_issue='approved' WHERE site_to_site_req.id ='".$_REQUEST['id']."';";
	
	echo $approve_update;
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("site_to_site_issue_approve_view","2");
		}
	}
 ?>
...
<div class="module_title">Site to Site Approve Requsition</div>
  ...
  <?php do{?>
    ...
    <tr>
      ...
      <td><?php echo $row_get_site_to_site_req['created_date']?></td>
      <td><a href=".?site_to_site_approve_item_view= true&amp;site_to_site_req_id=<?php echo $row_get_site_to_site_req['id']; ?>&back_page=site_to_site_issue_approve_view"><input name="" value="Items" type="submit" id="edit_forms"/></a></td>
            <td><a href=".?site_to_site_issue_approve_view&site_to_site_issue_approve=true&site_to_site_req_approve&id=<?php echo $row_get_site_to_site_req['id']; ?>"><input name="" value="Approve" type="submit" id="edit_forms"/></a></td>
    </tr>
