...
<?php
if(isset($_REQUEST['site_to_site_req_approve'])){
	$approve_update="UPDATE birus.site_to_site_req SET approved_date = '".date("y-m-d")."', approved_by = '".$loggedin_id."' WHERE site_to_site_req.id ='".$_REQUEST['id']."';";
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("site_to_site_req_approve_view","2");
		}
	}
 ?>
...
<div><a href="."><img src="images/back-button.png" width="30" height="30" title="Back" /></a></div> 
  ...
  <?php do{?>
    ...
    <tr>
      ...
      <td><a href=".?site_to_site_approve_item_view= true&amp;site_to_site_req_id=<?php echo $row_get_site_to_site_req['id']; ?>&back_page=site_to_site_req_approve_view"><input name="" value="Items" type="submit" id="edit_forms"/></a></td>
            <td><a href=".?site_to_site_req_approve_view&site_to_site_req_approve&id=<?php echo $row_get_site_to_site_req['id']; ?>"><input name="" value="Approve" type="submit" id="edit_forms"/></a></td>
    </tr>
