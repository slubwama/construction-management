...
$query_get_site_to_site_req = "
  ...
  FROM    (   (   birus.user user
       ...
       ON (site_to_site_req.site_id = site.id)
	   	  Where site_to_site_req.issued='false'
...
<?php 
if(isset($_REQUEST['site_to_site_req_issued'])){
	$approve_update="UPDATE birus.site_to_site_req SET issued_date = '".date("y-m-d")."', issued_by = '".$loggedin_id."',issued = 'true' WHERE site_to_site_req.id ='".$_REQUEST['id']."';";
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("site_to_site_req_issue_view","2");
		}
	}
 ?>
...
<div><a href="."><img src="images/back-button.png" width="30" height="30" title="Back" /></a></div> 
   <div class="module_title">Site To Site Issue</div>
  ...
  <?php $i=0; do{
    ...
    <tr>
      ...
      <td><a href=".?site_to_site_req_issue_view&site_to_site_req_issued=true&id=<?php echo $row_get_site_to_site_req['id']; ?>"><input name="" value="Issue" type="submit" id="edit_forms"/></a></td>
    </tr>
