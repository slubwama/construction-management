<?php get_main_menu("sitestorekeeper")?>
   ...
   <div style="width:90%; margin-left:auto; margin-right:auto;">
    ...
    <div class="box">
<?php 
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "site_req_item_form")  && (isset($_POST["save"]))) {
  $insertSQL = sprintf("INSERT INTO site_req_item (item_id, qty_required, site_req_id, unit_of_measure_id, created_by, created_date, record_status) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['item_id'], "int"),
                       GetSQLValueString($_POST['qty_required'], "double"),
                       GetSQLValueString($_POST['pr_id'], "int"),
                       GetSQLValueString($_POST['unit_of_measure_id'], "int"),
  ...
  if(mysql_affected_rows($birus_conn)){
	  	redirect_birus("site_req_item_view= true&site_req_id=".$_REQUEST['pr_id']."","2");
	  }
}
...
if (isset($_GET['id'])) {
  $colname_site_req_item_by_id = $_GET['id'];
}
mysql_select_db($database_birus_conn, $birus_conn);
$query_site_req_item_by_id = sprintf("SELECT * FROM site_req_item WHERE id = %s", GetSQLValueString($colname_site_req_item_by_id, "int"));
...
<div class="page_title_bar">
  <div class="page_title"><?php echo $_REQUEST['action']?> Site Requstion Item</div>
</div>
...
do {  
?>
        <option value="<?php echo $row_item['id']?>"<?php if (!(strcmp($row_item['id'], $row_site_req_item_by_id['item_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_item['name']?></option>
        <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td><input type="text" name="qty_required" value="<?php echo $row_site_req_item_by_id['qty_required']; ?>" size="32" /></td>
    </tr>
	</tr>
    <tr valign="baseline">
...
do {  
?>
        <option value="<?php echo $row_unit['id']?>"<?php if (!(strcmp($row_unit['id'], $row_site_req_item_by_id['unit_of_measure_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_unit['name']?></option>
        <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td><?php if(isset($_REQUEST['site_req_item_edit'])){	
		echo"<input name='update' type='submit' value='UPDATE' id='submit'/>";
					echo" <a href='.?site_req_item_view= true&site_req_id=".$_REQUEST['site_req_id']."&back_page=site_req_view'><input name='add' type='button' value='Cancel' id='submit'/></a>";
  ...
  <input type="hidden" name="pr_id" value="<?php echo $_REQUEST["site_req_id"] ?>" />
