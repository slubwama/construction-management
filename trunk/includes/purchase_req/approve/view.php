...
where purchase_req.approved_date='0000-00-00 00:00:00'
";
...
<?php
if(isset($_REQUEST['purchase_req_approve'])){
	$approve_update="UPDATE birus.purchase_req SET approved_date = '".date("y-m-d")."', approved_by = '".$loggedin_id."',approved='".$_REQUEST['approve_acttion']."' WHERE purchase_req.id ='".$_REQUEST['id']."';";
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("purchase_req_approve_view","2");
		}
	}
 ?>
...
<div><a href="."><img src="images/back-button.png" width="30" height="30" title="Back" /></a></div> 
  ...
  do{?>
    ...
    <tr>
      ...
      <td><a href=".?purchase_req_approve_view&purchase_req_approve&id=<?php echo $row_get_purchase_req['id']; ?>&amp;approve_acttion=approved"><input name="" value="Approve" type="submit" id="edit_forms"/></a></td>
      <td><a href=".?purchase_req_approve_view&purchase_req_approve&id=<?php echo $row_get_purchase_req['id']; ?>&amp;approve_acttion=declined"><input name="" value="Decline" type="submit" id="edit_forms"/></a></td>
    </tr>
