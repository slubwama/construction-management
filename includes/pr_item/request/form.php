...
<?php get_main_menu("storekeeper",$notification_store_keeper)?>
   ...
   <div style="width:90%; margin-left:auto; margin-right:auto;">
    ...
    <div class="box"> 
<?php
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "pr_item_form")  && (isset($_POST["save"]))) {
  $insertSQL = sprintf("INSERT INTO pr_item (item_id, qty_required, pr_id, unit_of_measure_id, created_by, created_date, record_status) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
if (isset($_GET['id'])) {
  $colname_pr_item_by_id = $_GET['id'];
}
mysql_select_db($database_birus_conn, $birus_conn);
$query_pr_item_by_id = sprintf("SELECT * FROM pr_item WHERE id = %s", GetSQLValueString($colname_pr_item_by_id, "int"));
...
<div class="page_title"><?php echo $_REQUEST['action']?> Purchase Requstion Item</div>
</div>
...
do {  
?>
        <option value="<?php echo $row_item['id']?>"<?php if (!(strcmp($row_item['id'], $row_pr_item_by_id['item_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_item['name']?></option>
        <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td><input type="text" name="qty_required" value="<?php echo $row_pr_item_by_id['qty_required']; ?>" size="32" /></td>
    </tr>
    <tr> </tr>
    <tr valign="baseline">
...
do {  
?>
        <option value="<?php echo $row_unit['id']?>"<?php if (!(strcmp($row_unit['id'], $row_pr_item_by_id['unit_of_measure_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_unit['name']?></option>
        <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td><?php if(isset($_REQUEST['pr_item_edit'])){	
		...
		echo" <a href='.?pr_item_view= true&purchase_req_id=".$_REQUEST['purchase_req_id']."&back_page=purchase_req_view'><input name='add' type='button' value='Cancel' id='submit'/></a>";
  ...
  <input type="hidden" name="id" value="<?php echo $row_pr_item_by_id['id']; ?>" />
