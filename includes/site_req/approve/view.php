...
SELECT person.first_name,
  ...
  FROM    (   (   birus.user user
           ...
           ON (site_req.created_by = user.id))
       INNER JOIN
          birus.site site
...
<?php
if(isset($_REQUEST['site_req_approve'])){
	$approve_update="UPDATE birus.site_req SET approved_date = '".date("y-m-d")."', approved_by = '".$loggedin_id."' WHERE site_req.id ='".$_REQUEST['id']."';";
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("site_req_approve_view","2");
		}
	}
 ?>
 ...
 <div><a href="."><img src="images/back-button.png" width="30" height="30" title="Back" /></a></div> 
  ...
  <?php do{?>
    ...
    <tr>
      ...
      <td><a href=".?site_approve_item_view= true&amp;site_req_id=<?php echo $row_get_site_req['id']; ?>&back_page=site_req_approve_view"><input name="" value="Items" type="submit" id="edit_forms"/></a></td>
            <td><a href=".?site_req_approve_view&site_req_approve&id=<?php echo $row_get_site_req['id']; ?>"><input name="" value="Approve Requsition" type="submit" id="edit_forms"/></a></td>
    </tr>
