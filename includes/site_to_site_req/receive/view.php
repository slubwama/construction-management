...
<?php 
if(isset($_REQUEST['site_to_site_req_recieved'])){
	$approve_update="UPDATE birus.site_to_site_req SET recieved_date = '".date("y-m-d")."', recieved_by = '".$loggedin_id."',recieved = 'true' WHERE site_to_site_req.id ='".$_REQUEST['id']."';";
	mysql_query($approve_update, $birus_conn) or die(mysql_error());
	if(mysql_affected_rows($birus_conn)>0){
			redirect_birus("site_to_site_req_recieve_view","2");
		}
	}
 ?>
<div><a href="."><img src="images/back-button.png" width="30" height="30" title="Back" /></a></div> 
   <div class="module_title">Site To Site Receive</div>
  ...
  <?php do{?>
    ...
    <tr>
      ...
      <td><a href=".?site_to_site_req_recieve_view&site_to_site_req_recieved&id=<?php echo $row_get_site_to_site_req['id']; ?>"><input name="" value="Receive" type="submit" id="edit_forms"/></a></td>
    </tr>
